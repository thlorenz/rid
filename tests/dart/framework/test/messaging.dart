...
void main() {
  test('messaging: isolate', () async {
    ...
    RidMsg msg = await rid.messageChannel.stream.first;
    expect(msg.toString(),
        'RidMsg{ type: RidMsgType.LogWarn, message: "Warn 0 from Rust" }');
  });
  test('messaging: log warn/info/debug which have no details', () async {
    ...
    expect(msg.toString(),
        'RidMsg{ type: RidMsgType.LogInfo, message: "Info 1 from Rust" }');

    rid_ffi.rid_export_send_log_debug_message(2);
    msg = await rid.messageChannel.stream.first;
    expect(msg.toString(),
        'RidMsg{ type: RidMsgType.LogDebug, message: "Debug 2 from Rust" }');
  });
  ...
  test('messaging: error/severe with details', () async {
    ...
    RidMsg msg = await rid.messageChannel.stream.first;
    expect(msg.toString(),
        'RidMsg{ type: RidMsgType.Error, message: "Error 1 from Rust", details: "Some Error Details" }');

    rid_ffi.rid_export_send_severe_error_message_with_details(2);
    msg = await rid.messageChannel.stream.first;
    expect(msg.toString(),
        'RidMsg{ type: RidMsgType.Severe, message: "Severe Error 2 from Rust", details: "Some Severe Error Details" }');
  });
  ...
  test('messaging: error/severe without details', () async {
    ...
    RidMsg msg = await rid.messageChannel.stream.first;
    expect(msg.toString(),
        'RidMsg{ type: RidMsgType.Error, message: "Error 1 from Rust" }');

    rid_ffi.rid_export_send_severe_error_message_without_details(2);
    msg = await rid.messageChannel.stream.first;
    expect(msg.toString(),
        'RidMsg{ type: RidMsgType.Severe, message: "Severe Error 2 from Rust" }');
  });
}
